...
class Closing_Sales_order
{
    ...
    public function select(stdClass $params)
    {
        ...
        $sql = "SELECT first 500 * from closing_so_s('$co_id') $par_search order by doc_date desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function update(stdClass $params)
    {
        ...
        if($params->status=='true'||$params->status=='1'){
            $params->status='1';
            $sql = "execute procedure closing_so_i '$timeinput','$userinput','$params->status','$params->remarks','$params->item_type','$params->unit_id','$params->item_id','$doc_date','$params->doc_id','$co_id'";
            $this->db->setSQL($sql);
            $this->db->execOnly();
            return $params;
        }
