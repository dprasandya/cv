...
class Account
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.Account')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT coa.*, status as active FROM coa where co_id='$co_id' $view_data $par_search order by coa_id asc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $idArray4 = $params->level;
        $array4 = [];
        for($i = 0 ; $i < count($idArray4) ; $i++){
            array_push($array4, "'".$idArray4[$i]."'");
        }
        if($idArray4<>''){
            $level =" and level IN(".implode(',',$array4).")";
        }else{
            $level =" and level=5";
        }

        $sql = "SELECT * FROM coa where co_id='$co_id' and status=1  $coa_type $station_type $station_group $level $par_search order by coa_id asc";
        //print_r($sql);
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
