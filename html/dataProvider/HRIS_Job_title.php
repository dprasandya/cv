...
class HRIS_Job_title
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.hris.employee.Job_title')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT hris_job_title.*, status as active FROM hris_job_title where co_id='$co_id' $view_data $par_search order by timeedit asc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * FROM hris_job_title where co_id='$co_id' $par_search order by job_id asc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'hris_job_title', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'hris_job_title', 'U', array('co_id'=>$params->co_id,'job_id' => $params->job_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from hris_job_title where co_id='$params->co_id' and job_id = '$params->job_id'");
        $this->db->execLog();
        return $params;
    }
