...
$file_name = $_FILES["filedata_ar_invoice-inputEl"]["name"];
$file_type = $_FILES["filedata_ar_invoice-inputEl"]["type"];
$file_size = round($_FILES["filedata_ar_invoice-inputEl"]["size"] / 1024, 2) . "  Kilo Bytes";
$slength = 0;
$sdelimiter = ",";
$senclosure = '';
$csvMimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
ini_set("auto_detect_line_endings", true);
if(!empty($_FILES['filedata_ar_invoice-inputEl']['name']) && in_array($_FILES['filedata_ar_invoice-inputEl']['type'],$csvMimes)) {

    if (is_uploaded_file($_FILES['filedata_ar_invoice-inputEl']['tmp_name'])) {

        //open uploaded csv file with read only mode
        $csvFile = fopen($_FILES['filedata_ar_invoice-inputEl']['tmp_name'], 'r');
        ...
        while(($line = fgetcsv($csvFile)) !== FALSE){
            if($line[1]){
            //  $sql = "execute procedure ar_invoice_upload_i('$timeinput','$userinput','$line[13]','$line[14]','$line[9]','$line[8]','$line[7]','$line[6]','$line[12]','$line[11]','$line[10]','$line[4]','$line[3]','$line[2]','$line[1]','$line[0]','$co_id')";
            //   print_r($sql);
                $prevQuery_x = $conn->query("execute procedure ar_invoice_upload_i('$timeinput','$userinput','$line[13]','$line[14]','$line[9]','$line[8]','$line[7]','$line[6]','$line[12]','$line[11]','$line[10]','$line[4]','$line[3]','$line[2]','$line[1]','$line[0]','$co_id')");
            }
