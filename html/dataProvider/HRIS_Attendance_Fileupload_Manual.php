...
$file_name = $_FILES["filedata_manual-inputEl"]["name"];
$file_type = $_FILES["filedata_manual-inputEl"]["type"];
$file_size = round($_FILES["filedata_manual-inputEl"]["size"] / 1024, 2) . "  Kilo Bytes";
$slength = 0;
$sdelimiter = ",";
$senclosure = '';
...
if(!empty($_FILES['filedata_manual-inputEl']['name']) && in_array($_FILES['filedata_manual-inputEl']['type'],$csvMimes)) {
    if (is_uploaded_file($_FILES['filedata_manual-inputEl']['tmp_name'])) {

        //open uploaded csv file with read only mode
        $csvFile = fopen($_FILES['filedata_manual-inputEl']['tmp_name'], 'r');
        ...
        while(($line = fgetcsv($csvFile)) !== FALSE){
            ...
            if($line[2]){ // emp_id //
                for ($x=0; $x<=count($array_date)-1; $x++)
                {
                    ...
                    if($array_date[$x]){
                        if($line[$status]){
                            $prevQuery_x = $conn->query("select count(*) as total from hris_attendance where co_id='$site' and emp_id='$line[2]' and attendance_date='$array_date[$x]'");
                            $prevQuery = $prevQuery_x->fetch(PDO::FETCH_ASSOC);
                            if($prevQuery[TOTAL] > 0) { // sudah pernah ada data //
                                $sql = "update hris_attendance set status='$line[$status]' where co_id='$site' and emp_id='$line[2]' and attendance_date='$array_date[$x]'";
                            }else{
                                $sql = "insert into hris_attendance(co_id, emp_id, attendance_date, status) values ('$site','$line[2]','$array_date[$x]','$line[$status]')";
                                //
                            }
                            $conn->query($sql);
                        }else{
                            $sql = "delete from hris_attendance where co_id='$site' and emp_id='$line[2]' and attendance_date='$array_date[$x]' ";
                            $conn->query($sql);
                        }
                    }
                }
