...
class TableFB
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.TableFB')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT table_fb.*, status as active FROM table_fb where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * FROM table_fb where co_id='$co_id' and status=1 $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'table_fb', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'table_fb', 'U', array('co_id'=>$params->co_id,'table_id' => $params->table_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from table_fb where co_id='$params->co_id' and table_id = '$params->table_id'");
        $this->db->execLog();
        return $params;
    }
