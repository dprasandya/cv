...
$file_name = $_FILES["filedata_ar_invoice_payment-inputEl"]["name"];
$file_type = $_FILES["filedata_ar_invoice_payment-inputEl"]["type"];
$file_size = round($_FILES["filedata_ar_invoice_payment-inputEl"]["size"] / 1024, 2) . "  Kilo Bytes";
$slength = 0;
$sdelimiter = ",";
$senclosure = '';
$csvMimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
ini_set("auto_detect_line_endings", true);
if(!empty($_FILES['filedata_ar_invoice_payment-inputEl']['name']) && in_array($_FILES['filedata_ar_invoice_payment-inputEl']['type'],$csvMimes)) {

    if (is_uploaded_file($_FILES['filedata_ar_invoice_payment-inputEl']['tmp_name'])) {

        //open uploaded csv file with read only mode
        $csvFile = fopen($_FILES['filedata_ar_invoice_payment-inputEl']['tmp_name'], 'r');

        //skip first line
        fgetcsv($csvFile); //, $slength, $sdelimiter, $senclosure
        //parse data from csv file line by line
        $co_id= $site; $userinput = $_SESSION['user']['usrname']; $timeinput = Time::getLocalTime('Y-m-d H:i:s') ; $doc_id = '';
        while(($line = fgetcsv($csvFile)) !== FALSE){
            if($line[1]){
            //  $sql = "select doc_id from ar_sale_payment_i('$timeinput','$userinput','0','$line[4]','$line[3]','$line[2]','P','$line[1]','$co_id')";
            //   print_r($sql);    
            $prevQuery_x = $conn->query("select doc_id from ar_sale_payment_i('$timeinput','$userinput','0','$line[4]','$line[3]','$line[2]','P','$line[1]','$co_id')");                
                $prevQuery = $prevQuery_x->fetch(PDO::FETCH_ASSOC);
                //print_r($prevQuery);
                $doc_id = $prevQuery['DOC_ID'];
                $sql_detail = $conn->query("execute procedure UPLOAD_AR_SALE_PAYMENT_DETAIL('$line[7]','$line[6]','$line[5]','$doc_id','$co_id')");
