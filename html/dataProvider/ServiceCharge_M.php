...
class ServiceCharge_M
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.ServiceCharge_M')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * FROM (
        select a.*, b.coa_name as coa_name_debit, c.coa_name as coa_name_credit, a.status as active from service_charge_m a 
        ...
        where co_id='$co_id' $view_data $par_search order by sc_id asc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * FROM service_charge_m where co_id='$co_id' and status=1 $par_search order by sc_id asc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'service_charge_m', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'service_charge_m', 'U', array('co_id'=>$params->co_id,'sc_id' => $params->sc_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from service_charge_m where co_id='$params->co_id' and sc_id = '$params->sc_id'");
        $this->db->execLog();
        return $params;
    }
