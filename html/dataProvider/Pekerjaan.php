...
class Pekerjaan
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.Pekerjaan')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT a.*, status as active FROM jenis_pekerjaan a where a.co_id='$co_id' $view_data $par_search order by a.timeedit desc";
        //print_r($sql);
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $sql = "SELECT * from jenis_pekerjaan where status=1 $par_search order by pekerjaan_id asc";
        //print_r($sql);
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'jenis_pekerjaan', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'jenis_pekerjaan', 'U', array('co_id'=>$params->co_id,'pekerjaan_id' => $params->pekerjaan_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from jenis_pekerjaan where co_id='$params->co_id' and pekerjaan_id = '$params->pekerjaan_id'");
        $this->db->execLog();
        return $params;
    }
}
