...
class Rounding
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.Rounding')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT a.*, status as active FROM rounding a where a.co_id='$co_id' $view_data $par_search order by a.timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'rounding', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'rounding', 'U', array('co_id'=>$params->co_id,'seq_id' => $params->seq_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from rounding where co_id='$params->co_id' and seq_id = '$params->seq_id'");
        $this->db->execLog();
        return $params;
    }
}
