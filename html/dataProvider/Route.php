...
class Route
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.Route')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT route.*, status as active FROM route where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'route', 'U', array('co_id'=>$params->co_id,'route_id' => $params->route_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from route where co_id='$params->co_id' and route_id = '$params->route_id'");
        $this->db->execLog();
        return $params;
    }
