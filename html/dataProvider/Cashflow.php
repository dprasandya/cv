...
class Cashflow
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.Cashflow')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT cashflow.*, status as active FROM cashflow where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $idArray = $params->cflow_type;
        $array = [];
        for($i = 0 ; $i < count($idArray) ; $i++){
            array_push($array, "'".$idArray[$i]."'");
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * FROM cashflow where co_id='$co_id' and status=1 and cflow_type IN(".implode(',',$array).")  $par_search order by cflow_id desc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'cashflow', 'U', array('co_id'=>$params->co_id,'cflow_id' => $params->cflow_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from cashflow where co_id='$params->co_id' and cflow_id = '$params->cflow_id'");
        $this->db->execLog();
        return $params;
    }
