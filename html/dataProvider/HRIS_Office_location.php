...
class HRIS_Office_location
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.hris.employee.Office_location')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT hris_office_locations.*, status as active FROM hris_office_locations where co_id='$co_id' $view_data $par_search order by timeedit asc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'hris_office_locations', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'hris_office_locations', 'U', array('co_id'=>$params->co_id,'ol_id' => $params->ol_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from hris_office_locations where co_id='$params->co_id' and ol_id = '$params->ol_id'");
        $this->db->execLog();
        return $params;
    }
