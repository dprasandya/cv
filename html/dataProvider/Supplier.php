...
class Supplier
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.Supplier')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT vendor.*, status as active FROM vendor where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $idArray = $params->vend_type;
        $array = [];
        for($i = 0 ; $i < count($idArray) ; $i++){
            array_push($array, "'".$idArray[$i]."'");
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * FROM vendor where co_id='$co_id' and status=1 and vend_type  IN(".implode(',',$array).") $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    public function delete(stdClass $params){
        $this->db->setSQL("delete from vendor where co_id='$params->co_id' and vend_id = '$params->vend_id'");
        $this->db->execLog();
        return $params;
    }
