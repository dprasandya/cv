...
class ExtraBed
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.ExtraBed')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "select extra_bed.*, extra_bed.status as active FROM extra_bed where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * from extra_bed where co_id='$co_id' and status=1 $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'extra_bed', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'extra_bed', 'U', array('co_id'=>$params->co_id,'bed_id' => $params->bed_id));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from extra_bed where co_id='$params->co_id' and bed_id = '$params->bed_id'");
        $this->db->execLog();
        return $params;
    }
