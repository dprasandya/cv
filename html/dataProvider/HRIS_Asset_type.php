...
class HRIS_Asset_type
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.hris.management_asset.Asset_type')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT hris_asset_type.*, status as active FROM hris_asset_type where co_id='$co_id' $view_data $par_search order by timeedit asc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function popup(stdClass $params)
    {
        ...
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT * FROM hris_asset_type where co_id='$co_id' $par_search order by asset_type asc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'hris_asset_type', 'I');
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $sql = $this->db->sqlBind($data, 'hris_asset_type', 'U', array('co_id'=>$params->co_id,'asset_type' => $params->asset_type));
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    public function delete(stdClass $params){
        $this->db->setSQL("delete from hris_asset_type where co_id='$params->co_id' and asset_type = '$params->asset_type'");
        $this->db->execLog();
        return $params;
    }
