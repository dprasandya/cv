...
class Salesman
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.master.Salesman')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT salesman.*, status as active FROM salesman where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function delete(stdClass $params){
        $this->db->setSQL("delete from salesman where co_id='$params->co_id' and sales_id = '$params->sales_id'");
        $this->db->execLog();
        return $params;
    }
