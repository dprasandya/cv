...
class AP_Invoice_buyback_unit
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.transactions.ap_invoice.AP_Invoice_buyback_unit')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT first 500 * FROM ap_invoice_s('$co_id') where co_id='$co_id' and doc_type='U' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
