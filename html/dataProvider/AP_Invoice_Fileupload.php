...
$file_name = $_FILES["filedata_ap_invoice-inputEl"]["name"];
$file_type = $_FILES["filedata_ap_invoice-inputEl"]["type"];
$file_size = round($_FILES["filedata_ap_invoice-inputEl"]["size"] / 1024, 2) . "  Kilo Bytes";
$slength = 0;
$sdelimiter = ",";
$senclosure = '';
$csvMimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
ini_set("auto_detect_line_endings", true);
if(!empty($_FILES['filedata_ap_invoice-inputEl']['name']) && in_array($_FILES['filedata_ap_invoice-inputEl']['type'],$csvMimes)) {

    if (is_uploaded_file($_FILES['filedata_ap_invoice-inputEl']['tmp_name'])) {

        //open uploaded csv file with read only mode
        $csvFile = fopen($_FILES['filedata_ap_invoice-inputEl']['tmp_name'], 'r');
        ...
        while(($line = fgetcsv($csvFile)) !== FALSE){
            if($line[1]){
            //  $sql = "select * from ap_invoice_i('$timeinput','$userinput',null,null,null,'$line[7]','$line[6]','$line[9]','$line[10]',0,'$line[11]',null, null, '$line[4]','$line[8]',null,null,null,'$line[3]','$line[2]','$line[1]','$co_id')";
            //   print_r($sql);
                $prevQuery_x = $conn->query("select * from ap_invoice_i('$timeinput','$userinput',null,null,null,'$line[7]','$line[6]','$line[9]','$line[10]',0,'$line[11]',null, null, '$line[4]','$line[8]',null,null,null,'$line[3]','$line[2]','$line[1]','$co_id')");
                $prevQuery = $prevQuery_x->fetch(PDO::FETCH_ASSOC);
                $doc_id = $prevQuery['DOC_ID'];
                //$sql_detail = $conn->query("execute procedure upload_ap_invoice_detail('$line[17]','$line[16]','$line[15]','$line[14]','$line[12]','$doc_id','$co_id')");
                $sql_detail = $conn->query("execute procedure ap_invoice_detail_i('$line[17]',null,'$line[16]','$line[15]','$line[14]','$line[13]','$line[12]',null,null,null,'$doc_id','$co_id')");
                //$sql = "execute procedure UPLOAD_AP_INVOICE('$userinput','$line[14]','$line[13]','$line[11]','$line[10]','$line[9]','$line[8]','$line[7]','$line[5]','$line[6]','$line[3]','$line[2]','$line[1]','$line[0]','$co_id')";
               // print_r($sql);
