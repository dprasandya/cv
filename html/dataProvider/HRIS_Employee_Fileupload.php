...
$file_name = $_FILES["filedata_employee-inputEl"]["name"];
$file_type = $_FILES["filedata_employee-inputEl"]["type"];
$file_size = round($_FILES["filedata_employee-inputEl"]["size"] / 1024, 2) . "  Kilo Bytes";
$slength = 0;
$sdelimiter = ",";
$senclosure = '';
...
if(!empty($_FILES['filedata_employee-inputEl']['name']) && in_array($_FILES['filedata_employee-inputEl']['type'],$csvMimes)) {
    if (is_uploaded_file($_FILES['filedata_employee-inputEl']['tmp_name'])) {

        //open uploaded csv file with read only mode
        $csvFile = fopen($_FILES['filedata_employee-inputEl']['tmp_name'], 'r');
        ...
        while(($line = fgetcsv($csvFile)) !== FALSE){
            //print_r($line);
            $co_id= $site;
            $birth_date = Date_Converter($line[7]); // BIRTH DATE
            $join_date = Date_Converter($line[23]); // JOIN DATE
            $userinput = $_SESSION['user']['usrname']; $timeinput = Time::getLocalTime('Y-m-d H:i:s') ;
            $group_id = $line[28]; $npwp = $line[27]; $ptkp_id = $line[26]; $payment_type = $line[25]; $st_name = $line[24]; $oh_id = $line[22]; $job_desc = $line[21]; $ol_name = $line[20]; $js_name = $line[19]; $pos_name = $line[18];
            $dept_name = $line[17]; $email = $line[16]; $mobile_phone = $line[15]; $address = $line[14]; $city = $line[13]; $blood = $line[12]; $nation = $line[11]; $marital_status = $line[10]; $religion = $line[9];
            $gender_name = $line[8]; $last_name = $line[6]; $midle_name = $line[5]; $first_name = $line[4]; $no_ktp = $line[3]; $nik = $line[2]; $id = $line[1]; $company_id = $line[0];
            //insert member data into database


            if($line[0]){ // kolom penomoran //
                $sql = "execute procedure hris_employee_upload('$timeinput','$userinput','$group_id','$npwp','$ptkp_id','$payment_type','$st_name','$join_date','$oh_id','$job_desc','$ol_name','$js_name','$pos_name','$dept_name','$email','$mobile_phone','$address','$city','$blood','$nation','$marital_status','$religion','$gender_name','$birth_date','$last_name','$midle_name','$first_name','$no_ktp','$nik','$id','$company_id','$co_id')";
                $conn->query($sql);
            }
        }
