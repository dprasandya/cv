...
class AR_Bad_debt_provision
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.transactions.ar_invoice.AR_Bad_debt_provision')<>1){
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = "execute procedure ar_sale_bad_debt_provision_i '$timeinput','$userinput','$params->status','$params->remarks','$params->cflow_id','$params->cash_id','$params->doc_type','$doc_date','$co_id'";
        $this->db->setSQL($sql);
        $this->db->execOnly();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $doc_date = $this->db->Date_Converter($params->doc_date); if($params->status==''){$params->status='0';}
        $sql = "execute procedure ar_sale_bad_debt_provision_u '$timeedit','$useredit','$params->status','$params->remarks','$params->cflow_id','$params->cash_id','$params->doc_type','$doc_date','$params->doc_id','$co_id'";
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
    ...
    public function selectdetail(stdClass $params)
    {
        ...
        $co_id = $_SESSION['user']['site'];
        $sql = "select a.*, a.status as active from ar_sale_bad_debt_provision_d_s('$params->percent','$params->aging_piutang','$params->doc_id','$co_id') a $par_search order by seq_id desc";
        $this->db->setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row) {
            $row = array_change_key_case($row);
    ...
    {
        $co_id = $_SESSION['user']['site'];
        if($params->active=='' or $params->active==false){$params->nominal=0;}
