...
$file_name = $_FILES["filedata_correction-inputEl"]["name"];
$file_type = $_FILES["filedata_correction-inputEl"]["type"];
$file_size = round($_FILES["filedata_correction-inputEl"]["size"] / 1024, 2) . "  Kilo Bytes";
$slength = 0;
$sdelimiter = ",";
$senclosure = '';
...
if(!empty($_FILES['filedata_correction-inputEl']['name']) && in_array($_FILES['filedata_correction-inputEl']['type'],$csvMimes)) {
    if (is_uploaded_file($_FILES['filedata_correction-inputEl']['tmp_name'])) {

        //open uploaded csv file with read only mode
        $csvFile = fopen($_FILES['filedata_correction-inputEl']['tmp_name'], 'r');
        ...
        while(($line = fgetcsv($csvFile)) !== FALSE){
            ...
            $co_id= $site;
            $correction_date = $line[0]; // CORRECTION DATE
            $userinput = $_SESSION['user']['usrname']; $timeinput = Time::getLocalTime('Y-m-d H:i:s') ;
            $emp_id = $line[1]; $emp_name = $line[2]; $nominal = $line[3]; $remarks = $line[4];
            //insert member data into database
           $sql = "execute procedure hris_salary_correction_upload('$timeinput','$userinput','1','$nominal','$remarks','$emp_id','$correction_date','$co_id')";
