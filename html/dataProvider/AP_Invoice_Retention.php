...
class AP_Invoice_Retention
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.transactions.ap_invoice.AP_Invoice_Retention')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT first 500 * FROM ap_invoice_retention_s('$params->doc_type','$co_id') where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = "execute procedure ap_invoice_i '$timeinput','$userinput','$params->warehouse_id','$params->for_doc_id','$tax_date','$params->tax_no','$params->status','$params->remarks','$params->registration_id','$params->unit_building_id','$params->vend_id','$params->tax_id','$params->costcode_id','$params->project_type','$params->project_id','$params->doc_type','$due_date','$doc_date','$co_id'";
        $this->db->setSQL($sql);
        $this->db->execOnly();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $doc_date = $this->db->Date_Converter($params->doc_date); if($params->status==''){$params->status='0';}
        $tax_date = $this->db->Date_Converter($params->tax_date); $due_date = $this->db->Date_Converter($params->due_date);
        $sql = "execute procedure ap_invoice_u '$timeedit','$useredit','$params->warehouse_id','$params->for_doc_id','$tax_date','$params->tax_no','$params->status','$params->remarks','$params->registration_id','$params->unit_building_id','$params->vend_id','$params->tax_id','$params->costcode_id','$params->project_type','$params->project_id','$params->doc_type','$due_date','$doc_date','$params->doc_id','$co_id'";
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
