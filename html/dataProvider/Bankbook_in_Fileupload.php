...
$file_name = $_FILES["filedata_bankbook_in-inputEl"]["name"];
$file_type = $_FILES["filedata_bankbook_in-inputEl"]["type"];
$file_size = round($_FILES["filedata_bankbook_in-inputEl"]["size"] / 1024, 2) . "  Kilo Bytes";
$slength = 0;
$sdelimiter = ",";
$senclosure = '';
$csvMimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
ini_set("auto_detect_line_endings", true);
if(!empty($_FILES['filedata_bankbook_in-inputEl']['name']) && in_array($_FILES['filedata_bankbook_in-inputEl']['type'],$csvMimes)) {

    if (is_uploaded_file($_FILES['filedata_bankbook_in-inputEl']['tmp_name'])) {

        //open uploaded csv file with read only mode
        $csvFile = fopen($_FILES['filedata_bankbook_in-inputEl']['tmp_name'], 'r');
        ...
        while(($line = fgetcsv($csvFile)) !== FALSE){
            if($line[1]){
                ...
                // print_r($sql);
                $prevQuery_x = $conn->query("select doc_id from bankbook_in_i('$timeinput','$userinput',0,'$line[7]',null,'$line[4]','$line[2]',null,'$line[5]','I','$line[1]',null,null,null,'$co_id')");                
