...
class Company
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.administration.Company')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $co_id = $_SESSION['user']['site'];
        $sql = "SELECT company.*, status as active FROM company where co_id='$co_id' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function delete(stdClass $params){
        $this->db->setSQL("delete from company where co_id='$params->co_id'");
        $this->db->execLog();
        return $params;
    }
