...
class HRIS_Leave_detail
{
    ...
    public function select(stdClass $params)
    {
        ...
        $sql = "SELECT * FROM hris_leave_detail_s('$params->ol_id','$params->lt_id','$user_id', '$co_id')  $par_search order by seq_id desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function add(stdClass $params)
    {
        ...
        unset($data['id'],$data['active'],$data['entitlement_pending'],$data['entitlement_take'],$data['entitlement_cancel']); $data['co_id']= $_SESSION['user']['site']; $data['userinput'] = $_SESSION['user']['name']; $data['timeinput'] = Time::getLocalTime('Y-m-d H:i:s') ;
    ...
    public function update(stdClass $params)
    {
        ...
        $fromdate = $this->db->Date_Converter($params->fromdate); if($params->status==''){$params->status='0';}
        $todate = $this->db->Date_Converter($params->todate);
        $sql = "execute procedure hris_leave_u '$timeedit','$useredit','$params->status','$params->remarks','$params->entitlement','$todate','$fromdate','$params->lt_id','$params->ol_id','$params->seq_id','$params->user_id','$params->emp_id','$co_id'";
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
