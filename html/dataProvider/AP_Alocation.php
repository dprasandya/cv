...
class AP_Alocation
{
    ...
    public function select(stdClass $params)
    {
        ...
        if($this->url->getPerm_Key_View($user_id,'App.view.transactions.ap_invoice.AP_Alocation')<>1){
            $view_data = "and userinput ='$user_usrname'";
        }
        $sql = "SELECT * FROM ap_invoice_payment where co_id='$co_id' and doc_type='$params->doc_type' $view_data $par_search order by timeedit desc";
        $this->db->setSQL($sql);
        $rows = array();
    ...
    public function add(stdClass $params)
    {
        ...
        $sql = "execute procedure ap_invoice_alocation_i '$timeinput','$userinput','$params->status','$params->remarks','$params->cflow_id','$params->cash_id','$params->doc_type','$doc_date','$co_id'";
        $this->db->setSQL($sql);
        $this->db->execOnly();
        return $params;
    }
    public function update(stdClass $params)
    {
        ...
        $doc_date = $this->db->Date_Converter($params->doc_date); if($params->status==''){$params->status='0';}
        $sql = "execute procedure ap_invoice_alocation_u '$timeedit','$useredit','$params->status','$params->remarks','$params->cflow_id','$params->cash_id','$params->doc_type','$doc_date','$params->doc_id','$co_id'";
        $this->db->setSQL($sql);
        $this->db->execLog();
        return $params;
    }
