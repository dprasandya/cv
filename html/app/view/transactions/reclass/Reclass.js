...
Ext.define('App.view.transactions.reclass.Reclass', {
    ...
    initComponent: function(){
        ...
        // Module Data Store
        // *************************************************************************************
        me.store = Ext.create('App.store.transactions.reclass.Reclass',{remoteSort: true});
        me.store_sharing_hpp = Ext.create('App.store.transactions.reclass.Reclass_sharing_hpp',{remoteSort: true});
        
        me.store_reclass = Ext.create('App.store.transactions.reclass.JurnalReclass',{remoteSort: true});
        ...
        me.grid = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                        ...
                        Ext.defer(function(){
                            new Ext.Button({
                                ...
                                handler : function(btn, e) {
                                    me.store_sharing_hpp.load({params:{doc_type:record.data.doc_type, total:record.data.total}});
                ...
                {text: 'Quantum CPO',flex: 1,sortable: true,dataIndex: 'qty_tbs'},
            ...
            tbar: [
                {
                    ...
                    listeners:{
                        specialkey:function(field, e){
                            if(e.getKey()== e.ENTER){
                                me.store.proxy.extraParams = {period:field.value, reclass_type:'FG' };
                                me.store.load();
                            }
                        }
                    }
                    ...
                    listeners:{
                        specialkey:function(field, e)
                        {
                            if(e.getKey()== e.ENTER)
                            {
                                ...
                                me.store.load({params:{period:period.getValue(), reclass_type:'FG', field_name:me.field_name, field_search:field.value, start:0, limit:15}})
                            }
                        }
                    }
                },
                    ...
                    listeners:{
                        specialkey:function(field, e)
                        {
                            if(e.getKey()== e.ENTER)
                            {
                                ...
                                me.store_reclass.load({params:{reclass_type:'FG', field_name:me.field_name, field_search:field.value, start:0, limit:15}})
                            }
                        }
                    }
                },'->',
                ...
                render: function() {
                    this.items.each(function(i, index, items){
                        i.tab.on('click', function(){
                            if(index==1){
                                i.store.load({params:{reclass_type:'FG'}});
                            }
