...
Ext.define('App.view.transactions.stock.Stock_out_non_project', {
    extend: 'App.ux.RenderPanel',
    pageTitle: _('stock_out_non_project'),
    initComponent: function(){
        ...
        // Module Data Store
        // *************************************************************************************
        me.store_detail = Ext.create('App.store.transactions.stock.Stock_out_detail',{remoteSort: true, groupField: 'formula_name'});
        me.store = Ext.create('App.store.transactions.stock.Stock_out_non_project',{remoteSort: true});
        ...
        me.grid_detail = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {text:_('stock'),width: 80,sortable: true,dataIndex: 'qty_stock', align:'right',renderer: Ext.util.Format.numberRenderer('0,000.00')},
        ...
        me.grid = Ext.create('Ext.grid.Panel', {
                                        ...
                                        render: function() {
                                            this.items.each(function(i, index, items){
                                                i.tab.on('click', function(){
                                                    if(index>0){
                                                        i.store.proxy.extraParams = {doc_date:me.data.doc_date, doc_id:me.data.doc_id, project_id:me.data.project_id, warehouse_id:me.data.warehouse_id, item_type:'HK'};
                                                        i.store.load();
                                                    }
            ...
            tbar: [
                ...
                },
                {
                    ...
                    width:100,
                    mode:'local',
                    store: [['project_name',_('project')],['doc_id',_('document')],['warehouse_name',_('warehouse')],['afdeling_name',_('afdeling')],['remarks',_('remarks')],['userinput',_('input_user')]],
                    listeners:{change:function(){me.field_name=this.getValue();}}
                },
                {
                    xtype:'textfield',
                    ...
                    width:150,
                    listeners:{specialkey:function(field, e){if(e.getKey()== e.ENTER){
                        me.store.proxy.extraParams = {doc_type:'N', field_name:me.field_name, field_search:field.value};
                        me.store.loadPage(1);}}
                    }
                },'->',
                {
    ...
    load_stock: function(){
        ...
        me.store_detail.proxy.extraParams ={project_type:'N', doc_date:me.data.doc_date, doc_id:me.data.doc_id, project_id:me.data.project_id, warehouse_id:me.data.warehouse_id, item_type:item_type.getValue(), start:0};
    ...
    onActive: function(callback){
        var me = this;
        me.store.proxy.extraParams ={doc_type:'N'};
        me.store.load();
        callback(true);
    }
});
