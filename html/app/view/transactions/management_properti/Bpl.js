...
Ext.define('App.view.transactions.management_properti.Bpl', {
    extend: 'App.ux.RenderPanel',
    pageTitle: _('bpl_utilities'),
    initComponent: function(){
        ...
        // Module Data Store
        // *************************************************************************************
        me.store = Ext.create('App.store.transactions.management_properti.Bpl',{remoteSort: true, pageSize : 20,  autoLoad: false});
        me.store_detail = Ext.create('App.store.transactions.management_properti.Bpl_detail',{remoteSort: true, pageSize : 20,  autoLoad: false});
        me.jurnal = Ext.create('App.view.transactions.jurnal.Jurnal');

        me.grid = Ext.create('Ext.grid.Panel', {
            store: me.store,
            title: _('bpl_utilities'),
            viewConfig :
            {
                stripeRows: false,
                ...
                me.formEditing = Ext.create('Ext.grid.plugin.RowEditing', {
                    ...
                    listeners: {
                        edit : function(editor, e) {
                            var using = e.record.data.qty_ending - e.record.data.qty_begin;
                            if(using < 0){
                                using = 0;
                                alert("qty ending can not be under qty begin ");
                            }
                            e.record.data.qty = using;
                            e.record.data.total = using * e.record.data.price;
                            e.record.data.tax_date = e.record.data.tax_date == null ? new Date() : e.record.data.tax_date;
            ...
            columns: [
                ...
                {text: _('type'),width: 80,sortable: true,dataIndex: 'bpl_name'},
                ...
                //{text: _('cluster'),width: 80,sortable: true,dataIndex: 'cluster_name'},
                ...
                {text: _('begin'),width: 80,sortable: true,dataIndex: 'qty_begin', align:'right',renderer: Ext.util.Format.numberRenderer('0,000.00')},
                {text: _('ending'),width: 80,sortable: true,dataIndex: 'qty_ending',renderer: Ext.util.Format.numberRenderer('0,000.00'), editor:{
                    xtype:'mitos.currency'
                }},
                {text: _('using'),width: 80,sortable: true,dataIndex: 'qty', align:'right',renderer: Ext.util.Format.numberRenderer('0,000.00')},
                ...
                {text: _('tax')+' No',width: 100,sortable: true,dataIndex: 'tax_no', editor:{
                    xtype:'textfield'
                }},
            ...
            tbar: [
                {
                    ...
                    listeners:{
                        specialkey:function(field, e){
                            var bpl = me.grid.down('toolbar').items.items[1],
                                tax_id = me.grid.down('toolbar').items.items[2];
                            if(e.getKey()== e.ENTER && bpl.getValue()!=null){
                                me.store.proxy.extraParams = {period:field.value, bpl_id:bpl.getValue(), tax_id:tax_id.getValue()};
                                me.store.loadPage(1);
                            }
                        }
                    }
                    ...
                    listeners:{
                        change:function(field, e){
                            ...
                            if(period.getValue()==null){
                                ...
                                me.store.proxy.extraParams = {period:period.getValue(), bpl_id:field.value, tax_id:tax_id.getValue() };
                                me.store.loadPage(1);
                            }
                        }
                    }
                },
                {
                    ...
                    listeners:{
                        change:function(field, e){
                            var period = me.grid.down('toolbar').items.items[0],
                                bpl_id = me.grid.down('toolbar').items.items[1];
                            if(period.getValue()!= null && bpl_id.getValue()!=null && field.value!=null ){
                                me.store.proxy.extraParams = {period:period.getValue(), bpl_id:bpl_id.getValue(), tax_id:field.value};
                                me.store.loadPage(1);
                            }
                        }
                    }
                ...
                },
                {
                    ...
                    listeners:{specialkey:function(field, e){if(e.getKey()== e.ENTER){
                        var period = me.grid.down('toolbar').items.items[0],
                            tax_id = me.grid.down('toolbar').items.items[2],
                            bpl = me.grid.down('toolbar').items.items[1];
                        me.store.proxy.extraParams = {period:period.getValue(), bpl_id:bpl.getValue(), tax_id:tax_id.getValue(), field_name:me.field_name, field_search:field.value};
                        me.store.loadPage(1);}}
        ...
        me.grid_detail = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {text: _('customer'),flex: 1,sortable: true,dataIndex: 'cust_name'},
                {text: 'Meter',width: 100,sortable: true,dataIndex: 'qty', align:'right',renderer: Ext.util.Format.numberRenderer('0,000.00')},
            ...
            tbar: [
                {
                    ...
                    width:100,
                    mode:'local',
                    store: [['doc_id',_('document')],['bpl_name',_('type')],['project_name',_('project')],['unit_building_name',_('unit_building')],['tax_id',_('tax')],['tax_no',_('tax')+' No'],['cust_name',_('customer')],['qty','Meter'],['price',_('price')],['receivable',_('amount')],['remarks',_('remarks')]],
                    listeners:{change:function(){me.field_name=this.getValue();}}
                },
                {
                    xtype:'textfield',
    ...
    onNewData: function(btn){
        ...
        for (var i = 0, len = length; i < len; i++) {
            ...
            Bpl.add(data, function(provider, response){
