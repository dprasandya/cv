Ext.define('App.view.popup.UnitBuildingCi',
    {
        extend : 'Ext.form.field.Trigger',
        alias : 'widget.xtunitbuildingci',
        ...
        initComponent : function()
        {
            ...
            me.grid = Ext.create('Ext.grid.Panel', {
                ...
                columns: [
                    ...
                    {text: _('room_rate'),width: 80,sortable: true,dataIndex: 'room_rate', align:'right',renderer: Ext.util.Format.numberRenderer('0,000.00')},
                    //{text: _('status'),width: 80,sortable: true,dataIndex: 'rent_status', align:'center'},
                    {text: _('room_service'),width: 100,sortable: true,dataIndex: 'room_service', align:'center'}
                ],
                tbar: [
                    {
                        xtype:'combo',
        ...
        gridclick: function(grid, selected){
            ...
            var me = this, form = this.up('container'), container = form.up('tabpanel'), plugin = container.ownerCt.editingPlugin,
                ...
                floor_id = plugin.editor.form.findField('floor_id'), price_brutto = plugin.editor.form.findField('price_brutto'),
                rt_id = plugin.editor.form.findField('rt_id'), rt_name = plugin.editor.form.findField('rt_name'),
                room_service = plugin.editor.form.findField('room_service');
            ...
            if(rt_id){rt_id.setValue(selected.data.rt_id);}
            ...
            if(rt_name){rt_name.setValue(selected.data.rt_name);}
            if(floor_id){floor_id.setValue(selected.data.floor_id);}
            if(price_brutto){price_brutto.setValue(selected.data.room_rate);}
            if(room_service){room_service.setValue(selected.data.room_service);}
