...
if(!defined('URI'))	define('URI', isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '/default/');
...
// application version
if(!defined('VERSION'))	define('VERSION', '0.0.102');
// extjs sdk directory
...
//$site = (isset($_GET['site']) ? $_GET['site'] : 'LSU');
$site = (isset($_SESSION['user']['site']) ? $_SESSION['user']['site'] : 'LSU');
...
//	$debug->debug('Test = ', ROOT. '/sites/' . $site . '/conf.php');
	// load modules hooks
		...
		foreach($modules as $module){
			...
			if(isset($module['styles'])){
				foreach($module['styles'] AS $style){
					$_SESSION['styles'][] = 'modules/' . $module['module_name'] . '/resources/css/' . $style;
				}
			}

			$HooksFile = ROOT . '/modules/' . $module['module_name'] . '/dataProvider/Hooks.php';
			if(!file_exists($HooksFile)) continue;

			include_once($HooksFile);
			$cls = 'modules\\'. $module['module_name'] .'\dataProvider\Hooks';
